...
public class Main {
    public static void main(String[] args) {
        ...
        BBallsGame solver = new BBallsGame();
        solver.solve(1, in, out);
        out.close();
    }

    static class BBallsGame {
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            int ans = 0;
            for (int i = 1; i < n; i++) {
                if (list.get(i).equals(list.get(i - 1)) && list.get(i) == x) {
                    TreeList<Integer> nList = new TreeList<>(list);
                    nList.add(i, x);
                    boolean f = true;
                    while (f) {
                        f = false;
                        ...
                        for (int j = 1; j < nList.size(); j++) {
                            if (nList.get(j).equals(nList.get(j - 1))) cnt++;
                            else {
                                if (cnt >= 3) {
                                    f = true;
                                    pos = j - cnt;
                    ...
                    ans = MaxMin.Max(ans, n - nList.size());
                }
            }
            out.println(ans);
        }
