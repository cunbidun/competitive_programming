...
public class Main {
    public static void main(String[] args) {
        ...
        DSuperM solver = new DSuperM();
        solver.solve(1, in, out);
        out.close();
    }

    static class DSuperM {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            for (int i = 1; i <= n; i++) {
                if (tree[i].size() == 1 && !ch[i]) q.add(i);
            }
            while (!q.isEmpty()) {
                int cur = q.poll();
                int parent = tree[cur].get(0);
                tree[parent].remove((Integer) cur);
                tree[cur].clear();
                if (tree[parent].size() == 1 && !ch[parent]) q.add(parent);
            }
            int pos = INF;
            int numNode = 0;
            for (int i = 1; i <= n; i++) {
                if (tree[i].size() > 0) numNode++;
                if (tree[i].size() == 1 && ch[i]) {
                    if (pos > i) pos = i;
                }
            }
            maxLevel = 0;
            ...
            int ans = INF;
            for (int i = 1; i <= n; i++) {
                if (level[i] == maxLevel) ans = MaxMin.Min(ans, i);
            }
            Arrays.fill(level, 0);
            pos = next;
            dfs(pos, pos);
