...
public class Main {
    public static void main(String[] args) {
        ...
        BParityAlternatedDeletions solver = new BParityAlternatedDeletions();
        solver.solve(1, in, out);
        out.close();
    }

    static class BParityAlternatedDeletions {
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            int n = in.nextInt();
            int[] a = in.nextIntArray(n, 1);
            TreeList<Integer> odd = new TreeList<>();
            TreeList<Integer> even = new TreeList<>();
            for (int i = 1; i <= n; i++) {
            ...
            if (odd.size() > 0) {
                odd.remove(0);
                for (int i : odd) sum1 += i;
            } else if (even.size() > 0) {
                for (int i : even) sum1 += i;
            }
            ...
            if (odd.size() > 0) for (int i : odd) sum2 += i;
            else if (even.size() > 0) {
                even.remove(0);
                for (int i : even) sum2 += (long) i;
            }
            out.println(MaxMin.Min(sum1, sum2));
        }
