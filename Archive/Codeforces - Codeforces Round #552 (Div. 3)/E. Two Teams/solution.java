...
public class Main {
    public static void main(String[] args) {
        ...
        ETwoTeams solver = new ETwoTeams();
        solver.solve(1, in, out);
        out.close();
    }

    static class ETwoTeams {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            STRangeSum sgTree = new STRangeSum(n);
            for (int i = 1; i <= n; i++) {
                int x = a[i];
                tree.add(x);
                q.add(x);
            ...
            while (!q.isEmpty()) {
                ...
                int curPos = pos[cur] - sgTree.getSum(pos[cur], pos[cur]);
                curPos--;
//            for (int i : tree) {
//                out.print(i + " ");
//            }
//            out.println();
                for (int i = MaxMin.Min(curPos + k, tree.size() - 1); i >= MaxMin.Max(0, curPos - k); i--) {
                    int tmp = tree.get(i);
                    if (cnt % 2 == 1) ans[tmp] = 1;
                    else ans[tmp] = 2;
                    sgTree.update(pos[tmp], n, 1);
                    tree.remove(i);
                }
                cnt++;
            }
    ...
    static class STRangeSum {
        private int[] segment;
        private int[] lazy;
        private int[] array;
        private int N;

        public STRangeSum(int n) {
            segment = new int[4 * n];
            lazy = new int[4 * n];
            N = n;
        }

        public STRangeSum(int n, int[] inputArray) {
            segment = new int[4 * n];
            lazy = new int[4 * n];
        ...
        public STRangeSum(int n, int[] inputArray, int MOD) {
            segment = new int[4 * n];
            lazy = new int[4 * n];
            this.array = inputArray;
            N = n;
        ...
        private void buildTree(int i, int l, int r, int MOD) {
            ...
            segment[i] = (int) Op.addMod(segment[i * 2], segment[i * 2 + 1], MOD);
        }
