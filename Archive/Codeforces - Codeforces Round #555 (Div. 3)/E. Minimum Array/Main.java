...
public class Main {
    public static void main(String[] args) {
        ...
        EMinimumArray solver = new EMinimumArray();
        solver.solve(1, in, out);
        out.close();
    }

    static class EMinimumArray {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            int[] c = new int[n + 1];
            for (int i = 1; i <= n; i++) {
                int pos = BinarySearch.lowerBound(list, n - a[i]);
                if (pos == list.size()) {
                    pos = 0;
                }
                c[i] = (a[i] + list.get(pos)) % n;
                list.remove(pos);
            }
            for (int i = 1; i <= n; i++) {
                out.print(c[i] + " ");
    ...
    static class BinarySearch {
        public static int lowerBound(TreeList<Integer> array, int indexSt, int indexEn, int value) {
        ...
        public static int lowerBound(TreeList<Integer> array, int value) {
            return lowerBound(array, 0, array.size() - 1, value);
        }
