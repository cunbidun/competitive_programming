...
public class Main {
    ...
    static class DAlmostAllDivisors {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            while (t-- > 0) {
                int n = in.nextInt();
                TreeList<Long> list = new TreeList<>();
                for (int i = 1; i <= n; i++) {
                ...
                for (long i : list) {
                    ans = MathAlgorithm.lcm(ans, i);
                    if (ans > 10000000000000L) {
                        d = true;
                        out.println(-1);
                        break;
                    }
                ...
                if (!d) {
                    if (ans == list.get(list.size() - 1)) ans *= list.get(0);
                }
                if (!d)
                    for (long j = 2; j <= Math.sqrt(ans); j++) {
                        if (ans % j == 0) {
                            if (!list.contains(j) || !list.contains(ans / j)) {
                                out.println(-1);
                                d = true;
                                break;
                            }
