/**
 *  author: cunbidun
 *  created: Sunday, 2023-07-30 22:53:06 CDT
 **/
#include <array>
#include <assert.h>
#include <functional>
...
int main() {
  ...
  // f[i][j] = the number of satisfied way to arrange binary numbers
  // if only consider the first i elements, and j is the last element has different value
  // j = 0 means all the number are equal
  vector<vector<mint>> f(N + 1, vector<mint>(N + 1));
  if (a[1][1] != 2) {
    f[1][0] = 2;
  }
  auto check = [&](int c, int last) -> bool {
    for (int i = c; i >= 0; i--) {
      if (i > last && a[i][c] == 2) {
        return false;
      }
