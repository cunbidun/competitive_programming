/**
 *  author: cunbidun
 *  created: Sunday, 2023-07-16 16:31:53 CDT
 **/
#include <array>
#include <assert.h>
#include <functional>
...
int main() {
  ...
  mint_nck<(int)1e9 + 7> C(N);
  mint ans = 0;
  auto compute_part_k = [&](int part, int k) -> mint {
    assert(part <= k);
    return C.get(k - part + part - 1, part - 1);
  };
  for (int i = 1; i <= min({K, N - K}); i++) {
    ans += C.get(N - K, i) * mint(3).pow(K - i) * mint(3).pow(N - K - i) * compute_part_k(i, K);
  }

  for (int i = 0; i <= min(K - 1, N - K); i++) {
    ans += C.get(N - K, i) * mint(3).pow(K - (i + 1)) * mint(3).pow(N - K - i) * compute_part_k(i + 1, K);
  }
  cout << ans << '\n';
}
