/**
 *  author: cunbidun
 *  created: Sunday, 2023-01-29 20:27:10 CST
 **/
#include <array>
#include <assert.h>
#include <functional>
...
int main() {
  ...
  const int MOD = 1e9 + 7;
  using mint = mint<MOD>;
  auto C = mint_nck<MOD>(2e5 + 5);
  vector<mint> new_start(H - A + 1);
  for (int i = 1; i <= H - A; i++) {
    new_start[i] = C.get(B + i - 2, B - 1);
  }
  mint ans = 0;
  for (int i = 1; i <= H - A; i++) {
    ans += new_start[i] * C.get(W - B + (H - i + 1) - 2, W - B - 1);
  }
  cout << ans << '\n';
}
